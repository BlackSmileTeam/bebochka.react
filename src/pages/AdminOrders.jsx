...
function getAdminStatusSelectOptions(currentStatus) {
  ...
  if (currentStatus === 'Отправлено частично') return ['Отправлено частично']
...
const STATUS_TOOLTIPS = {
  ...
  'Копим': 'Заказ в накоплении: ждём дополнительные позиции перед оплатой или сборкой',
  ...
  'Отправлено частично': 'Часть заказа уже отправлена, остальное ещё собираем. Статус меняется автоматически',
...
function AdminOrders() {
  ...
  const performStatusChange = async (orderId, newStatus, confirmSplit = false) => {
    try {
      setUpdatingStatuses(prev => new Set(prev).add(orderId))
      await api.updateOrderStatus(orderId, newStatus, confirmSplit)
      await loadOrders()
      setSelectedOrders(prev => {
        const next = new Set(prev)
        next.delete(orderId)
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    ...
    if (order && shouldOfferSplitOrder(order, currentStatus, newStatus)) {
      setStatusChangeWarning({
        type: 'split',
        orderId,
        newStatus,
        text: 'Не все вещи нашли на складе. Разбить заказ на две отправки? В первую пойдут отмеченные «В посылке», остальные останутся в сборке.'
      })
      return
    }
  ...
  const handleConfirmStatusWarning = async () => {
    ...
    if (warning.type === 'split') {
      await performStatusChange(warning.orderId, warning.newStatus, true)
      return
    }
                  ...
                  <table className="orders-table">
                    ...
                    <tbody>
                      {sortOrdersInGroup(statusOrders).map(order => {
                        ...
                        const parentOrderId = getParentOrderId(order)
                        const isChildOrder = parentOrderId != null && parentOrderId !== ''

                        return (
                          <Fragment key={orderId}>
                            <tr
                              className={`order-data-row order-data-row--clickable ${isSelected ? 'row-selected' : ''} ${orderRowMenuOpen === orderId ? 'row-menu-open' : ''} ${isChildOrder ? 'order-data-row--child' : ''}`.trim()}
