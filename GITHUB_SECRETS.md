...
### 5. VITE_API_URL (важно для same-origin)

**Не задавайте** секрет `VITE_API_URL` в GitHub (Settings → Secrets → Actions).

Если секрет уже есть со значением `https://bebochka.ru` — **удалите его**.  
Иначе фронт на `:55502` снова будет звать API на 443, а не через свой nginx.

При пустом секрете сборка использует `window.location.origin` + прокси `/api` в `nginx.conf`.
